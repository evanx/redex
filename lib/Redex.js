...
export default class Redex {
      ...
      for (let processorName of [...this.processorKeys]) {
         ...
         if (lodash.isFunction(processor.init)) {
            ...
            try {
               ...
               logger.error('init', processorName);
               throw err;
            }
         } else {
      ...
      for (let processorName of [...this.processorKeys]) {
         ...
         if (lodash.isFunction(processor.start)) {
            ...
            try {
               ...
               logger.error('start', processorName);
               throw err;
            }
         } else {
   ...
   endProcessors() {
      for (let processorName of [...this.processorKeys.reverse()]) {
         let processor = this.processorMap.get(processorName);
   ...
   start() {
      ...
      if (this.config.env.cancelled) {
         logger.warn('start cancelled', this.config);
         this.endProcessors();
         return false;
      }
      logger.info('started', this.config);
      this.monitorId = setInterval(() => this.monitor, 1000);
