...
const RedexFiles = {
   ...
   formatClassType(classType, className) {
      return util.format('%s/%s/%s.js', RedexGlobal.sourceDir, classType, className);
   },
   formatConfigurator(configuratorClass) {
      return RedexFiles.formatClassType('configurator', configuratorClass);
   },
   formatProcessor(processorClass) {
      return RedexFiles.formatClassType('processor', processorClass);
   },
