...
const logger = bunyan.createLogger({name: 'RedexConfigs', level: RedexGlobal.loggerLevel});
...
function summariseProcessorConfig(config) {
  let summary = Object.assign({ label: config.label }, config);
  ['processorName', 'startup', 'instance', 'class', 'loggerLevel', 'label'].forEach(key => {
      delete summary[key];
  });
  return Object.assign({ label: config.label }, summary);
}
