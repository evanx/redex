...
async function getConfigObject(env) {
   ...
   if (env.configFile) {
      ...
      assert(fs.statSync().isDirectory(), 'baseDir: ' + env.baseDir);
      env.configFile = Paths.join(env.baseDir, 'redex.yaml');
      if (!env.configDir) {
         env.configDir = env.baseDir + '/config';
         assert(fs.statSync().isDirectory(), 'configDir: ' + env.configDir);
      }
   ...
   assert(fs.statSync().isFile(), 'configFile: ' + env.configFile);
