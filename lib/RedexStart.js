...
let state = {};
let logger = bunyan.createLogger({name: 'RedexStart', level: 'debug'});
...
async function start() {
   try {
      ...
      if (state.redexConfig.metaConfig) {
         logger.info('metaConfig', state.redexConfig.metaConfig);
      }
...
async function getConfigObject(env) {
   ...
   if (env.configurator) {
      ...
      let metaConfigFile = util.format('configurator.%s.default.yaml', env.configurator);
      let metaConfigPath = util.format('%s/config/%s', global.redexDir, metaConfigFile);
      console.log('metaConfigPath', metaConfigPath);
   ...
   redexConfig.configFile = env.configDir;
