...
import RedexMetaConfigs from './RedexMetaConfigs';
...
async function start() {
   try {
      ...
      RedexProcessors.createProcessors(that.redex, that.redexConfig.configs);
      that.redex.init();
...
async function getConfigObject(env) {
   ...
   if (fileName.indexOf(RedexFiles.configuratorPrefix) === 0) {
      if (!env.configurator) {
         ...
         let classFile = RedexFiles.formatConfigurator(configuratorClass);
         logger.debug('check:', classFile);
         if (await Files.exists(classFile)) {
...
async function invokeConfigurator(configuratorClass) {
   ...
   let metaConfig = RedexMetaConfigs.decorate(readFileSyncYamlFile(metaConfigPath));
...
async function getConfigsFromConfigurator(configuratorClass, config) {
   ...
   let configurator = require(RedexFiles.formatConfigurator(configuratorClass));
