...
async function start() {
   try {
      ...
      delete state.redexConfig.configs;
   } catch (e) {
...
async function getConfigObject(env) {
   ...
   if (env.configurator) {
      console.log('redexDir', global.redexDir);
      let configuratorConfigFile = util.format('configurator.%s.default.yaml', env.configurator);
      let configuratorConfigPath = util.format('%s/config/%s', global.redexDir, configuratorConfigFile);
      console.log('configuratorConfigPath', configuratorConfigPath);
      let configuratorConfig = readFileSyncYamlFile(configuratorConfigPath);
      console.log('configuratorConfig', configuratorConfig);
      let configs = await getConfigsFromConfigurator(configuratorConfigFile, configuratorConfig);
      return Object.assign({configuratorConfig}, {
         configurator: env.configurator,
         configs: configs
      });
   }
   if (!env.configurator) {
