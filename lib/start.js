...
var logger, redix, envConfig, redixConfig;

start();

async function start() {
   try {
      envConfig = extractConfigFromEnv(process.env);
      console.log('envConfig', envConfig);
      redixConfig = await getConfig(envConfig);
      ...
      global.redix = redix = new Redix(redixConfig);
...
async function getConfig(env) {
   ...
   assert(env.configDir, 'env.configDir');
   assert(env.configFile, 'env.configFile');
   console.log('getConfig', env);
   return Object.assign(env, await getConfigFromFile(env.configFile));
}
